	...
	{	
		...
		if($customer_id)
		{
			...
			$data['sales_until_discount'] = ($this->config->item('number_of_sales_for_discount')) ? (float)$this->config->item('number_of_sales_for_discount') - (float)$cust_info->current_sales_for_discount : 0;
                ...
                $require_customer_for_payment = $this->config->item('require_customer_for_sale_by_payment');
                $required_payment_type = $this->config->item('require_customer_for_sale_by_payment_type');
                $has_required_payment_type = FALSE;
                $payments = isset($data['payments']) ? $data['payments'] : $this->cart->get_payments();

                if ($require_customer_for_payment && $required_payment_type)
                {
                        $required_payment_labels = array_unique(array_filter(array(
                                $required_payment_type,
                                lang($required_payment_type),
                        )));

                        $payment_language_map = $this->Sale->get_payment_options_with_language_keys();

                        foreach($payments as $payment)
                        {
                                $payment_label = $payment->payment_type;

                                if (($payment_label_colon_pos = strpos($payment_label, ':')) !== FALSE)
                                {
                                        $payment_label = trim(substr($payment_label, 0, $payment_label_colon_pos));
                                }

                                if (in_array($payment_label, $required_payment_labels))
                                {
                                        $has_required_payment_type = TRUE;
                                        break;
                                }

                                foreach($payment_language_map as $current_language_label => $language_key)
                                {
                                        if (($payment_label === $current_language_label || $payment_label === lang($language_key)) && $required_payment_type === $language_key)
                                        {
                                                $has_required_payment_type = TRUE;
                                                break 2;
                                        }
			...
			if ($has_required_payment_type && !$customer_id)
			{
