...
class Locations extends Secure_area implements Idata_controller
{
	...
	*/
	...
	function view($location_id=-1,$redirect=false)
	{
		...
		// Location creation no longer requires remote license validation or purchase email.
	...
	function location_auth_check($email)
		{
			// License validation has been disabled; always allow.
			return TRUE;
		}
	...
	function save($location_id=-1)
	{
		...
		'sidekick_api_key'=>$this->input->post('sidekick_api_key'),
		'sidekick_auto_review' => $this->input->post('sidekick_auto_review') ? 1 : 0,
		'coreclear_merchant_id' => $this->input->post('coreclear_merchant_id'),
		'blockchyp_terms_and_conditions' => $this->input->post('blockchyp_terms_and_conditions'),
		'blockchyp_work_order_pre_auth' => $this->input->post('blockchyp_work_order_pre_auth'),
		'blockchyp_work_order_post_auth' => $this->input->post('blockchyp_work_order_post_auth'),
		'blockchyp_prompt_for_loyalty' => $this->input->post('blockchyp_prompt_for_loyalty') ? 1 : 0,
		'blockchyp_prompt_for_name' => $this->input->post('blockchyp_prompt_for_name') ? 1 : 0,
